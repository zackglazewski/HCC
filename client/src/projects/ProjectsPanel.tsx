...
export function ProjectsPanel({
  ...
  return (
    <div className="fixed inset-0 bg-black/30 flex items-start justify-center p-8 z-50">
          ...
          <ul className="divide-y">
            {cards.map((c) => (
              <li key={c.id} className="py-2 flex items-center gap-2">
                <button className="flex-1 text-left" onClick={() => onOpenCard(c)}>
                ...
                <button className="px-2 py-1 border rounded" onClick={() => onDeleteClick(c)}>
                  Delete
                </button>
              </li>
            ))}
